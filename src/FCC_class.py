...
class FCCSigmoid(nn.Module):
    ...
    def __init__(self, units):
        ...
        :param units: a list [u1,u2] where u1 is the # of units in the 1st hidden layer and u2 in the 2nd hidden layer
        """
        super(FCCSigmoid, self).__init__()
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1)
        x = torch.sigmoid(self.fLayer1(x))  # input layer to first hidden layer
        x = torch.sigmoid(self.fLayer2(x))  # first hidden layer to second hidden layer
